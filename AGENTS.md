...
### 核心规则(2026-07-29 用户亲自定下,优先级最高)

1. **先讨论后编码**:业务逻辑/数据模型的任何变更,必须先输出"人话版"方案(加什么表/字段/页面、为什么、有什么取舍),经用户确认后才写代码。
2. **小步输出**:每次只做一小块工作,做完停下来等用户验收,再进行下一块。不要一次输出大量代码或连续执行多个动作。
3. **决策留痕**:讨论得出的结论记录进 `docs/PROGRESS.md`;关键业务问题(学费怎么记、奖学金挂哪级、角色权限等)由用户拍板,AI 只提供选项和建议,不擅自决定。
