...
## 2026-08-21 — UI 改版施工中(1-3/7)+ 校园照片管线 ✅

### 完成

- **设计地基**:字体自托管(Onest + Playfair Display,西里尔子集,public/fonts/,经 Clash 下载);设计 token 写入 globals.css(米白底 #F7F6F5/近黑 #141414/点缀红 #D91F1F/灰蓝 #6C7E85/发丝线);动效类 rise-in(逐词浮现)/settle-in(图片 5s 沉降);深色自动切换已关(浅色优先)
- **全局壳**:毛玻璃吸顶导航 + 页脚;lucide-react 图标库接入,emoji 开始退役
- **首页 v1**:超大细字重标语逐词浮现("Найди свой университет в Китае")+ 实时数据区(64 校/183 项目/实时汇率)+ 精选学校 + 奖学金入口条。已验收
- **学校列表页**(目录索引式,方案 A 用户选定):编号+发丝线行、校徽、hover 红名+箭头滑入、下划线极简筛选器、生活费入副标、"双一流"俄文化(Двойной первоклассный)。已验收
- **学校详情页**:聚焦布局(左内容/右吸顶费用规格表)、目录行项目与奖学金、emoji 清零。已验收
- **校园照片管线**(Wikimedia Commons CC 图):脚本 `scripts/fetch-photos.py` 搜索→下载→webp 压缩(225KB/张)→灰底拼图人工过目→按"门面→全景→教学→生活"叙事排序写入。3 校试点 16 张上线。坑:upload.wikimedia.org 有 429 限流,需慢速+重试;Wikimedia 主站国内直连可达
- **照片画廊组件** `photo-gallery.tsx`:页内直接浏览——悬停缩略图切主图、主图两侧箭头(悬停浮现)/触屏滑动、当前红框高亮(用户拍板的交互,放弃点进灯箱方案)
- **项目详情页改版**(参数/费用发丝线规格表、吸顶右栏、黑色胶囊申请 CTA,已验收)
- **奖学金目录页改版**(衬线大标题、下划线 tab 筛选、目录行、截止胶囊)
- **修复中文泄漏**:Scholarship 加 `applicationChannelRu` 字段(迁移已执行),翻译脚本+覆盖率报告已纳入
- **互动中国地图上线**(首页招牌):纯 SVG 方案(MapLibre 与 Turbopack worker 不兼容,改为 GeoJSON→SVG 预生成,零运行时依赖);亚洲+俄罗斯世界视野(Natural Earth 18 国衬底);点省份填红+底部学校横滑列表;点学校右侧飞出速览面板(校徽/校名/城市/简介/CTA;面板打开时地图不动、底部列表保留可切换)

### 下一步(UI 施工剩余)

- [x] ~~小页面改版~~(登录/注册:衬线标题+下划线输入框+胶囊按钮;收藏页:目录行+空态引导;新增俄语 404 页)
- [x] ~~移动端细查~~(390px:导航精简防溢出、全局 overflow-x 防护;列表/详情/画廊均正常)
- [ ] 照片管线铺到全部已发布学校(注意:seed 三校 hit/blcu 没走过 logo 管线,logo 缺;swjtu 用户手传过)
- [ ] 数据放量:其余学校项目灌入(全量+校验)
- [ ] 品牌/域名
- [ ] 登录/注册页、收藏页、404 页改版(小页面)

## 2026-08-19 — 哈工大全量灌入 + 数据判读规则确立 ✅

### 完成

- **哈工大本科全量灌入(93 条草稿)**:中文段 65 个专业(xls 67 个除 2 个艺考类,**程序化从原始 xls 生成**,零 LLM 挑选)+ 英文轨道 28 条(英文段 30 个除艺考);工商管理英文轨道一删一恢复,最终以原始文件核对为准
- **西交大/东北师大全量化(同日)**:xjtu 49 条(42 中文+5 英文+MBBS;从存档的简章正文程序化解析,学费按学位类别分档映射)、nenu 23 条(31 个除 7 艺考+1 编导;PDF 提取文本带断行,程序修复)。三所试点全部放弃"精选",全量覆盖
- **费用填满**:中文 20000/英文 26000、保险 800/年、申请费 400、截止 2026-07-15;住宿费区间按用户拍板**取区间下限转年价**(800-1000 元/月 → 9600/年);nenu 保险同步补 800
- 俄文翻译同步(150+ 字段),覆盖率报告 100%;对齐校验全绿
- 导入脚本支持全部费用字段;去重按"专业名+(英文授课)后缀"约定区分语言轨道

### 决策(数据判读规则,用户多轮校准后确立)

1. **判读一律回原始网页/附件**,加工过的中间文件(提取文本、JSON)不作为依据
2. **简章概括文字 vs 附件表格不一致时,以附件表格为准**;在备注/要求里留出处
3. **全量灌入取代精选**(用户拍板):LLM 挑选主流专业会漏会错(俄语专业就曾被漏);全量+机器校验+抽查成本更低。既定排除项保留:艺考类等
4. 双语专业建模:维持"拆两条目"(因中英文轨道学费不同,单条目无法展示两个价格);远期如需"单条目双价"再改 schema
5. **艺考类专业:录入**(用户拍板 2026-08-19)——核实后确认国际生申艺考类仅多"作品集/面试"增量(西交简章原文佐证),不走国内艺考统考,没理由排除。回填后三校目录 100% 覆盖:hit 97(67中+30英)、nenu 31、xjtu 49(44中+5英)

## 2026-08-18 — UI 改版设计定调(文档产出,未施工)

- **设计方向拍板**:极简画廊 + 互动招牌(中国地图);反面清单:emoji 图标、横向长卷、重 3D
- **4 个参考站实测拆解**(fundaacps/findrealestate/realestateabbey/travel-sensations):共识配方=暖米白底+近黑文字+单一红点缀+发丝线+无阴影+慢动效;各站借鉴点、字体(全部换西里尔安全免费字体:Onest/Playfair Display)、动效实现难度、页面方案、施工顺序全部固化在 **`docs/04-设计系统与UI改版.md`**(施工依据,同蓝图地位)
- ⚠️ fundaacps/travel-sensations 两站被恶意劫持,勿直接打开;截图存档 `design-refs/`(已 gitignore)

## 2026-08-17(下)— 俄语化 + 前台零中文 + 翻译管线 ✅

### 完成

- **schema v2.2 俄文字段**:Program +requirementsRu/scholarshipNoteRu;Scholarship +nameRu/coverageRu/descriptionRu;University +strongDisciplinesRu。后台表单全部双语并排
- **DeepSeek 翻译管线**(`pnpm translate`,deepseek-v4-flash):扫描"有中文没俄文"字段→翻译→写回;已翻跳过、失败重试、断点续跑;**收尾自动输出已发布内容的俄文覆盖率报告**(防遗漏机制)。共翻 230+ 字段,覆盖率 100%
- **前台零中文(用户定原则)**:名称俄文优先英文兜底;简介/要求/说明/优势学科只显示俄文,没有就隐藏;省市中俄映射 `geo.ts`(新城市入库时审核环节补映射);"双一流"→Двойной первоклассный;后台保持中文
- 代码审查清理:删死代码 getProgram();lint/tsc 全绿;前台页面全 200

### 决策

- 翻译用 DeepSeek API(deepseek-v4-flash)而非占主对话 token:便宜、可反复跑;key 在 .env(不入库);模型可用性以 /models 接口实测为准
- 前台语言纪律:**前台零中文,后台全中文**;没翻译的内容隐藏不显示,不混语言

## 2026-08-16~17 — 灌数据第二层试点 + 对齐校验系统 ✅

### 完成

- **10 校招生页结构调研**(清华/北大/南大/西交大/北外/西南交大/东北师大/黑大 + 哈工大/上交大),结论三类:A 类单页全量(西交大/北外/东北师大)、B 类简章+附件目录(哈工大 xls/南大 PDF/清华)、C 类麻烦(北大学费未定价、西南交大附件 503、黑大整站不可达)。无一家有登录墙;费用普遍"按层次+学科+授课语言分档统一价";授课语言普遍逐专业标注(无人标俄语授课——市场缺口实锤)
- **试点三校 53 个项目草稿**:xjtu 20 / nenu 18 / hit 15(+5 去重跳过),`prisma/data/programs/*.json` + `import-programs.ts`(校级公共字段 common + 专业条目,幂等去重)。附件原件存 `prisma/data/attachments/` 备查
- **对齐校验系统(审核提效核心)**:`src/modules/admin/verify.ts` 确定性字符串比对(零 LLM)——草稿关键字段 vs 官网简章页文本+本地附件文本,归一化后逐字段核对(专业名/学费/截止日/授课语言;学校:校名/城市/官网可达)。触发:审核页「🔄 脚本审核」按钮 或 `pnpm verify`。报告 `verify-report.json` → 审核页徽章。首战成果:53 项目全绿;60 学校 54 绿 6 红(全是已知反爬站+城市名弱校验噪音)
- **审核页重构为学校分组工作台**:学校→项目父子层级,组头一键「学校+内容全部发布」(publishUniversityGroup,事务批量);全绿组按钮绿色有红变灰;平台级奖学金独立区块
- **项目管理页折叠化**(就地编辑,删除独立编辑页);项目/奖学金表单加来源「访问 ↗」;摘要行加授课语言标签
- **日期时区 bug 修复(校验脚本抓出)**:date-only 字段按 UTC 零点存,负时区服务器读出差一天 → 统一按 UTC 中午 12 点存(`parseDateOnly`),存量 55 条批量矫正

### 决策

- 审核提效路线(用户拍板):分级信任——机器对齐(脚本,非 LLM 自证)+ 人工只看红色异常 + 全绿整组批发;LLM 生成字段(简介/俄文名/优势学科)不对齐,走人工/合伙人复核
- 对齐校验定位:保证"忠于原文",不保证"原文最新";年度重抓对比兜底
- 放量策略:用户审完当前批次后再放量;层次先只做本科;批次顺序按结构友好度优先
- **工具迁移**:用户计划迁往 Claude Code 继续开发,项目上下文以 AGENTS.md + 本文件为准(2026-08-17 交接提交)

### 下一步(交接给后续对话)

- [ ] 用户核查发布 60 所学校 + 53 个项目(用新的对齐徽章+整组发布)
- [ ] 核查完毕后放量灌数据:每批 10 所,本科优先,A/B 类结构友好学校优先;C 类人工补丁列表(北大/西南交大/黑大)
- [ ] ustc/lzu 校徽人工上传(表单内有搜图链接);西南交大的双一流勾选
- [ ] 硕士层次数据(本科铺满后);奖学金详情;校园照片(Wikimedia)
- [ ] 数据健康:lastVerifiedAt 超期标黄;数据预检标红(预检异常自动标注,对齐的下一阶段)
- [ ] UI 改版、品牌/域名、部署(海外服务器优先)
