...
## 2026-09-22(补)— 校徽白色版事故修复:48 所换彩色版

### 问题(用户验收发现)

前台大量校徽"空白":v2 管线抓到的多是**官网深色页眉用的白色版校徽**(透明底白字),而此前校审拼图用灰底,白校徽在灰底上清晰可见所以全数通过;站点背景是米白,白色版直接隐形。共 46 所中招(38 PNG + 8 个 Wikimedia 纯白 SVG),另终检又揪出 cau/fudan 两个浅灰描边版,合计 **48 所**。

### 修复(scripts/bing-logos-color.py)

- 必应中文搜「校名 校徽」,**新增自动过滤**:候选合成到纯白底后彩色/深色像素占比 < 2.5% 判为白色版丢弃;尺寸/长宽比不合格也丢
- **校审拼图改用纯白底**(与站点真实背景一致),逐校选定彩色版;46 校每校 3 候选,cau/fudan 后补
- 48 校全部换为彩色 PNG(缩至 ≤480px);8 所原 SVG 学校 DB 路径改为 PNG,旧 SVG 已删
- **终检自动化**:全量 64 校 logo 合成白底跑同一过滤,0 漏网——以后新校徽入库都应过这一关

### 教训(管线纪律)

- 图片"内容正确"≠"在站点上可见":透明底素材的审核底衬必须与最终展示背景一致
- 素材入库前置校验(白底可见性)已证明必要,照片管线如有透明素材同理

## 2026-09-22 — 照片/校徽管线重做为必应中文搜索(旧管线废止)

### 背景(用户发现的严重问题)

旧照片管线(Wikimedia 英文搜索)错图系统性偏多:北理工照片实为北京石油化工学院、北师大混进首师大附中、搜索串味。根因:英文校名撞名 + 拼图太小没看清图中文字。用户拍板:**准确性优先,全部重做**。

### 新管线(scripts/bing-photos.py, scripts/bing-logos.py)

- **搜索源:必应中文图片搜索**「校名 校园/大门」「校名 校徽」——中文关键词直接锁定中国学校,相关度排序首条基本就是标志性校门
- **坑**:桌面 UA 连续查询会被必应降级成 stock 占位图,换手机 UA 解决
- **我逐张视觉审核**(不是小拼图扫一眼):大门必须能读出校名;64 校 427 张照片**零错图**;64 校校徽同样**零错图**
- 校徽缺口补齐:ustc/lzu/dlufl 用新搜到的替换;blcu/hit/swjtu 三所 seed/手动录入校(从没进过管线)校徽补齐入库——**64 校 logoUrl 全部非空**

### 结果

- **64 所学校全部有校园照片**(webp 压缩,已写库);3 所 seed 校(bit/bnu/csu 试点已验证)并入同管线
- 旧 Wikimedia 管线(fetch-all-photos.py)废止,产物已清

## 2026-09-16 — 数据放量批次 1 + 照片铺满 ✅

### 完成

- **批次 1:10 所学校 606 条项目草稿**(sjtu/tsinghua/pku/nju/bfsu/fudan/zju/tongji/ecnu/whu),全部子代理调研+原始附件解析,全部留疑点备注;全量收录(含艺考类)。至此 13 所学校有项目数据(试点 3 + 本批 10)
- **对齐校验加固**(`verify.ts`):附件自动转文本(pdf/xls/docx/html)纳入比对语料;日期增加多种格式变体(含英文 "March 6, 2026");修复全角括号截取 bug(专业名后缀)。当前 606 条中 405 全绿;剩余 201 红全部是工具极限:**ecnu**(PDF 数字乱码)、**whu**(.doc 老格式费用表)、**tsinghua 美院 17 条**(美院专业名来源页未抓到)——这三类需人工核一眼
- **校园照片铺满**:56/57 校抓到(336 张),压缩 webp(均 225KB);我逐张视觉复核(8 张拼图)剔除地铁站/串味/非校园错图;**52 校写入数据库 + 试点 3 校 = 55 校有照片**;7 所归人工(清华/同济/川大/东大/西农/电子科大/北二外)。工程化:`scripts/fetch-all-photos.py`(慢速防 429)
- ⚠️ **DeepSeek 402 余额耗尽**,批次 1 的翻译中断待充值续跑(断点续翻)→ **后续**:充值两次跑完大部分,剩余 550 条由子代理直接翻译写回(`i18n-out/` + `apply-translations.ts`),覆盖率报告确认 100%。翻译管线双通道成型:API 优先,LLM 子代理兜底

### 决策

- 校验徽章语义:绿=机器已核对;红=看 note,红不代表数据错,可能是"机器核不了"(附件未转文本/PDF 乱码)——人只处理红
- 照片沿用 B 方案:拼图人工过目后才写库

### 下一步

- [ ] 用户充值 DeepSeek → 续跑翻译(pnpm translate)
- [ ] 批次 2:剩余学校项目灌入(结构友好优先)
- [ ] 7 所学校照片人工补(编辑页有搜图链接);blcu/swjtu 两所 seed 校也补照片
- [ ] 审核批次 1(606 条,重点看各校"疑点")
