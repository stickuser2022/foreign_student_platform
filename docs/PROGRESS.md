...
## 2026-08-02 — /admin 第二轮开工:项目录入 + 编辑 ✅
...
- **项目管理** `/admin/programs`(列表:名称/学校/层次/状态/最近核实)+ **编辑页** `/admin/programs/[id]/edit`(预填现有数据,保存即生效)。**已验收**(改学费前台即时生效,状态不变)
- 录入/编辑共用表单组件 `src/modules/admin/program-form.tsx`
- 数字输入框加 `min={0}`(学制允许小数 step="any");学校录入的生活费同样修正
- 看板加「录入新项目 →」「项目管理 →」入口
...
- [ ] 项目列表页 + 编辑页(/admin/programs、/admin/programs/[id]/edit)
- [ ] 学校/奖学金的列表页 + 编辑页
- [ ] 奖学金录入表单
