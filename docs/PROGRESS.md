...
- [x] ~~实时汇率~~(money.ts:俄罗斯央行 CBR 每日官方汇率,服务器缓存 6h + 10s 超时 + 兜底 11;学校/项目/收藏三页已改异步,费用卡显示「Курс ЦБ РФ」;**本机直连 CBR 被墙,本地开发显示兜底值属正常,海外服务器部署后即真实汇率**)
...
- [ ] UI 改版(时机:第一批数据灌入后、对外推广前;shadcn/ui 组件库 + 用户提供参考站,逐页验收;/admin 不美化)
